...
namespace AdoHelp
{
    class SQLHelps
    {
        static readonly string connStr = ConfigurationManager.ConnectionStrings["sqlConStr"].ConnectionString;
        ...
        public static int ExecuteNonQuery(string cmdText, CommandType type, params SqlParameter[] paras)
        {
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                ...
                {                 
                    ...
                    conn.Open();
                    SqlTransaction st = conn.BeginTransaction(IsolationLevel.ReadCommitted);   //共享锁
                    cmd.Transaction = st;  //事务 
