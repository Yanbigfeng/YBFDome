...
namespace Ado
{
    public class SqlMethod
    {
        ...
        #region 调用使用的操作示例
        public static void sqlUserExp()
        {
            SqlDataReader read = null;
            int Count = 0;
            //方法调用
            //根据范围查询
            //  Count = SqlMethod.Between("BaseTable", "age","16","19");
            //查询数据库所有表名
            //dateread = SqlMethod.SearchTable();
            //查询表所有的列名
            //read = SqlMethod.SearchColumn("BaseTable");
            //储存过程
            // read= SqlMethod.ProcBaseTable("proc_BaseTable");
            //事物操作
            SqlMethod.UpdateNo("update BaseTable set name='张三' where name=@name", new SqlParameter("@name", "'小七'"));
            SqlMethod.Save();
            //表的关联操作
            read = SqlMethod.Union();
            //详情打印
            while (read.Read())
